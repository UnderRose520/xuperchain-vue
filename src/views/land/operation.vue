<template>
  <div v-if="name == '国土资源局管理员'" class="dashboard-container">
    <div class="head">
      土地使用证信息
    ...
    <el-dialog title="添加信息" :visible.sync="dialogFormVisible" width="800px">
      <el-form :model="add">
        <el-form-item label="使用者" :label-width="searchLabelWidth"><el-input v-model="add.useName" autocomplete="off" style="width: 500px" /></el-form-item>
        ...
        <el-form-item label="地号" :label-width="searchLabelWidth"><el-input v-model="add.landNumber" autocomplete="off" style="width: 500px" /></el-form-item>
        <el-form-item label="用途" :label-width="searchLabelWidth"><el-input v-model="add.purpose" autocomplete="off" style="width: 500px" /></el-form-item>
        <el-form-item label="使用期限" :label-width="searchLabelWidth"><el-input v-model="add.serviceLife" autocomplete="off" style="width: 500px" /></el-form-item>
      </el-form>
    ...
    <el-table :data="tableData" style="width: 100%" max-height="1000" :row-class-name="tableRowClassName" border stripe>
      <el-table-column fixed prop="data.useName" label="使用者" width="200" align="center" />
      <el-table-column prop="data.address" label="地址" width="200" align="center" />
      <el-table-column prop="data.landNumber" label="地号" width="350" align="center" />
      <el-table-column prop="data.purpose" label="用途" width="350" align="center" />
      <el-table-column prop="data.serviceLife" label="使用期限" width="200" align="center" />
    ...
    <el-dialog title="证书预览和下载" :visible.sync="dialogVisible" width="60%" :before-close="handleClose">
      <div id="pdfDom">
        <div class="proBox">
          <p class="tit">超级链国土资源局证书</p>

          <p class="con">
            使用者：
            <span class="con-name">{{ CA.useName }}</span>
          ...
          <p class="con">地号：{{ CA.landNumber }}</p>
          <p class="con">用途：{{ CA.purpose }}</p>
          <p class="con">使用期限：{{ CA.serviceLife }}</p>
...
export default {
  ...
  methods: {
    ...
    addInfo() {
      this.landAddForm.userid = this.add.userid
      this.landAddForm.useName = this.add.useName
      this.landAddForm.address = this.add.address
      this.landAddForm.landNumber = this.add.landNumber
      this.landAddForm.purpose = this.add.purpose
      this.landAddForm.serviceLife = this.add.serviceLife
      axios
        .post('http://41c484d417.zicp.vip/v1/landAdd', qs.stringify(this.landAddForm), { headers: { 'Content-Type': 'application/x-www-form-urlencoded' }})
        .then(res => {
          ...
          this.CA.useName = this.data.useName
          this.CA.userid = this.data.userid
          this.CA.address = this.data.address
          this.CA.landNumber = this.data.landNumber
          this.CA.purpose = this.data.purpose
          this.CA.serviceLife = this.data.serviceLife
