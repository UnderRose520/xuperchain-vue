<template>
  <div v-if="name == '城乡规划部门管理员'" class="dashboard-container">
    <div class="head">
      规划许可证信息
    ...
    <el-dialog title="添加信息" :visible.sync="dialogFormVisible" width="800px">
      <el-form :model="add">
        <el-form-item label="建设单位" :label-width="searchLabelWidth"><el-input v-model="add.buildUnite" autocomplete="off" style="width: 500px" /></el-form-item>
        ...
        <el-form-item label="建设位置" :label-width="searchLabelWidth"><el-input v-model="add.buildLocation" autocomplete="off" style="width: 500px" /></el-form-item>
        <el-form-item label="建设规模" :label-width="searchLabelWidth"><el-input v-model="add.buildScale" autocomplete="off" style="width: 500px" /></el-form-item>
    ...
    <el-table :data="tableData" style="width: 100%" max-height="1000" :row-class-name="tableRowClassName" border stripe>
      <el-table-column fixed prop="data.buildUnite" label="建设单位" width="200" align="center" />
      <el-table-column prop="data.projectname" label="项目名称" width="200" align="center" />
      <el-table-column prop="data.buildLocation" label="建设位置" width="350" align="center" />
      <el-table-column prop="data.buildScale" label="建设规模" width="350" align="center" />
    ...
    <el-dialog title="证书预览和下载" :visible.sync="dialogVisible" width="60%" :before-close="handleClose">
      <div id="pdfDom">
        <div class="proBox">
          <p class="tit">超级链城乡规划部证书</p>

          <p class="con">
            建设单位：
            <span class="con-name">{{ CA.buildUnite }}</span>
            <span class="code">编号：{{ CA.code }}</span>
          </p>
          <p class="con">
            项目名称：
            <span>{{ CA.projectname }}</span>
          </p>
          <p class="con">建设位置：{{ CA.buildLocation }}</p>
          <p class="con">建设规模：{{ CA.buildScale }}</p>
...
export default {
  ...
  methods: {
    ...
    addInfo() {
      this.urbanRuralAddForm.buildUnite = this.add.buildUnite
      this.urbanRuralAddForm.projectname = this.add.projectname
      this.urbanRuralAddForm.buildLocation = this.add.buildLocation
      this.urbanRuralAddForm.buildScale = this.add.buildScale
      this.urbanRuralAddForm.issueDate = this.add.issueDate
      this.urbanRuralAddForm.userid = this.add.userid
      axios
        .post('http://41c484d417.zicp.vip/v1/urbanRuralAdd', qs.stringify(this.urbanRuralAddForm), { headers: { 'Content-Type': 'application/x-www-form-urlencoded' }})
        .then(res => {
          ...
          this.CA.buildUnite = this.data.buildUnite
          this.CA.projectname = this.data.projectname
          this.CA.buildLocation = this.data.buildLocation
          this.CA.buildScale = this.data.buildScale
