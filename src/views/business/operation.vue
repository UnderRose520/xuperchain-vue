<template>
  <div v-if="name == '工商局管理员'" class="dashboard-container">
    <div class="head">
      营业执照信息
    ...
    <el-dialog title="添加信息" :visible.sync="dialogFormVisible" width="800px">
      <el-form :model="add">
        <el-form-item label="名称" :label-width="searchLabelWidth"><el-input v-model="add.name" autocomplete="off" style="width: 500px" /></el-form-item>
        ...
        <el-form-item label="负责人" :label-width="searchLabelWidth"><el-input v-model="add.charger" autocomplete="off" style="width: 500px" /></el-form-item>
        <el-form-item label="地址" :label-width="searchLabelWidth"><el-input v-model="add.address" autocomplete="off" style="width: 500px" /></el-form-item>
        <el-form-item label="经营范围" :label-width="searchLabelWidth"><el-input v-model="add.businessScope" autocomplete="off" style="width: 500px" /></el-form-item>
        <el-form-item label="经营期限" :label-width="searchLabelWidth"><el-input v-model="add.operatingPeriod" autocomplete="off" style="width: 500px" /></el-form-item>
      </el-form>
    ...
    <el-table :data="tableData" style="width: 100%" max-height="1000" :row-class-name="tableRowClassName" border stripe>
      <el-table-column fixed prop="data.name" label="名称" width="200" align="center" />
      <el-table-column prop="data.userid" label="身份证" width="200" align="center" />
      <el-table-column prop="data.charger" label="负责人" width="150" align="center" />
      <el-table-column prop="data.address" label="地址" width="350" align="center" />
      <el-table-column prop="data.businessScope" label="经营范围" width="200" align="center" />
      <el-table-column prop="data.operatingPeriod" label="经营期限" width="150" align="center" />
    ...
    <el-dialog title="证书预览和下载" :visible.sync="dialogVisible" width="60%" :before-close="handleClose">
      <div id="pdfDom">
        <div class="proBox">
          <p class="tit">超级链工商局证书</p>

          <p class="con">
            负责人：
            <span class="con-name">{{ CA.charger }}</span>
          ...
          <p class="con">经营范围：{{ CA.businessScope }}</p>
          <p class="con">经营期限：{{ CA.operatingPeriod }}</p>
...
export default {
  ...
  methods: {
    ...
    addInfo() {
      this.businessAddForm.userid = this.add.userid
      this.businessAddForm.name = this.add.name
      this.businessAddForm.businessScope = this.add.businessScope
      this.businessAddForm.address = this.add.address
      this.businessAddForm.charger = this.add.charger
      this.businessAddForm.operatingPeriod = this.add.operatingPeriod
      axios
        .post('http://41c484d417.zicp.vip/v1/businessAdd', qs.stringify(this.businessAddForm), { headers: { 'Content-Type': 'application/x-www-form-urlencoded' }})
        .then(res => {
          ...
          this.CA.charger = this.data.charger
          this.CA.address = this.data.address
          this.CA.businessScope = this.data.businessScope
          this.CA.operatingPeriod = this.data.operatingPeriod
