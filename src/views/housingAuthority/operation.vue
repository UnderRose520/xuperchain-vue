<template>
  <div v-if="name == '房管局管理员'" class="dashboard-container">
    <div class="head">
      商品房预售许可证信息
    ...
    <el-dialog title="添加信息" :visible.sync="dialogFormVisible" width="800px">
      <el-form :model="add">
        <el-form-item label="预售人" :label-width="searchLabelWidth"><el-input v-model="add.preSeller" autocomplete="off" style="width: 500px" /></el-form-item>
        ...
        <el-form-item label="项目名称" :label-width="searchLabelWidth"><el-input v-model="add.projectName" autocomplete="off" style="width: 500px" /></el-form-item>
        <el-form-item label="常房售号" :label-width="searchLabelWidth"><el-input v-model="add.usualSaleNum" autocomplete="off" style="width: 500px" /></el-form-item>
        <el-form-item label="预售面积" :label-width="searchLabelWidth"><el-input v-model="add.preArea" autocomplete="off" style="width: 500px" /></el-form-item>
        <el-form-item label="签发日期" :label-width="searchLabelWidth"><el-input v-model="add.issueDate" autocomplete="off" style="width: 500px" /></el-form-item>
      </el-form>
    ...
    <el-table :data="tableData" style="width: 100%" max-height="1000" :row-class-name="tableRowClassName" border stripe>
      <el-table-column fixed prop="data.preSeller" label="预售人" width="200" align="center" />
      <el-table-column prop="data.preArea" label="预售面积" width="200" align="center" />
      <el-table-column prop="data.projectName" label="项目名称" width="350" align="center" />
      <el-table-column prop="data.usualSaleNum" label="常房售号" width="350" align="center" />
      <el-table-column prop="data.issueDate" label="签发日期" width="200" align="center" />
    ...
    <el-dialog title="证书预览和下载" :visible.sync="dialogVisible" width="60%" :before-close="handleClose">
      <div id="pdfDom">
        <div class="proBox">
          <p class="tit">超级链房管局证书</p>

          <p class="con">
            预售人：
            <span class="con-name">{{ CA.preSeller }}</span>
            <span class="code">编号：{{ CA.code }}</span>
          </p>
          <p class="con">
            预售面积：
            <span>{{ CA.preArea }}</span>
          </p>
          <p class="con">项目名称：{{ CA.projectName }}</p>
          <p class="con">常房售号：{{ CA.usualSaleNum }}</p>
          <p class="con">签发日期：{{ CA.issueDate }}</p>
...
export default {
  ...
  data() {
    return {
      ...
      data: '',
      hash: '',
      housingAuthorityAddForm: {
        functionName: 'addHousingAuthority',
        userid: '',
        preSeller: '',
        preArea: '',
        projectName: '',
  ...
  methods: {
    ...
    addInfo() {
      this.housingAuthorityAddForm.userid = this.add.userid
      this.housingAuthorityAddForm.preArea = this.add.preArea
      this.housingAuthorityAddForm.preSeller = this.add.preSeller
      this.housingAuthorityAddForm.projectName = this.add.projectName
      this.housingAuthorityAddForm.usualSaleNum = this.add.usualSaleNum
      this.housingAuthorityAddForm.issueDate = this.add.issueDate
      axios
        .post('http://41c484d417.zicp.vip/v1/housingAuthorityAdd', qs.stringify(this.housingAuthorityAddForm), { headers: { 'Content-Type': 'application/x-www-form-urlencoded' }})
        .then(res => {
          ...
          this.CA.preSeller = this.data.preSeller
          this.CA.userid = this.data.userid
          this.CA.preArea = this.data.preArea
          this.CA.projectName = this.data.projectName
          this.CA.usualSaleNum = this.data.usualSaleNum
          this.CA.issueDate = this.data.issueDate
